...
import { createModule, getModulesByCourseSlug } from '@/api/services/module';
...
function ContentFilters({
  ...
  const mutation = useMutation({
    mutationFn: (data: CreateModulePayload) => createModule(data, courseSlug),
...
function ContentTable({ modules }: { modules: Module[] }): React.JSX.Element {
  ...
  const { data } = useQuery({
    queryKey: ['modules'],
    queryFn: () =>
      getModulesByCourseSlug(courseSlug, {
          ...
          {table.getRowModel().rows.map((row) => {
            return (
              <TableRow
                ...
                onClick={() => {
                  router.push(
                    `/dashboard/course/${courseSlug}/content/${row.original.id}`,
                  );
                }}
              >
